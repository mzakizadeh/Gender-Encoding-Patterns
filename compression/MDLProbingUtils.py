...
def general_probing_args(parser):
    parser.add_argument(
        "--seed", type=int, help="the random seed to check on", required=True
    )
    parser.add_argument(
        "--model_seed",
        type=int,
    ...
    parser.add_argument(
        "--embedding_size", type=int, help="embedding size", default=768
    )
    parser.add_argument(
        "--batch_size", type=int, help="batch size to train the probe", default=32
    )
    parser.add_argument(
        "--probe_type", type=str, help="linear probe or MLP", choices=["linear", "mlp"]
    )


def general_MDL_args():
    parser = argparse.ArgumentParser(
        description="Probe vectors for gender of example using MDL probes."
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        help="linear probe of MLP",
        default=[2.5, 3.0, 4.4, 6.5, 9.5, 14.0, 21.0, 31.0, 45.7, 67.6, 100],
    )
...
def run_MDL_probing(args, load_fn, task_name, shuffle):
    ...
    online_code_probe = OnlineCodeMDLProbe(
        lambda: create_probe(args), args.mdl_fractions, device=device
    )

    train_dataset, val_dataset, test_dataset = load_fn(args)

    folder = f"data/MDL_Results/{args.experiment_name}/{args.training_data}"
    ...
    reporting_root = join(
        os.getcwd(),
        f"data/MDL_Results/online_coding_{task_name}_{args.type}_{args.experiment_name}_{args.max_length}_layer_{layer}.pt",
    )

    uniform_cdl, online_cdl = online_code_probe.evaluate(
        train_dataset,
        test_dataset,
        val_dataset,
