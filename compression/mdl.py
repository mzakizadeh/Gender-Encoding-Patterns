...
class OnlineCodeMDLProbe(MDLProbe):
    ...
    @staticmethod
    def split_datasets(
        dataset: Union[Dataset, Sized], fractions: List[float], shuffle=False
    ):
        ...
        if isinstance(Dataset, IterableDataset):
            raise ValueError(
                "dataset must not be of type InstanceDataset, and must implement a __getitem__ method"
            )
        # Normalize to [0, 1]
        ...
        for i in range(len(fractions)):
            if shuffle:
                train_subset = Subset(
                    dataset, p[range(0, int(fractions[i] * total_len))]
                )
            else:
                # train_subset = Subset(dataset, range(0, int(fractions[i] * total_len)))
                train_subset["y"] = Subset(
                    dataset["y"], range(0, int(fractions[i] * total_len))
                )
                train_subset["z"] = Subset(
                    dataset["z"], range(0, int(fractions[i] * total_len))
                )
            ...
            if i != len(fractions) - 1:
                ...
                if shuffle:
                    eval_subset = Subset(
                        ...
                        p[
                            range(
                                int(fractions[i] * total_len),
                                int(fractions[i + 1] * total_len),
                            )
                        ],
                    )
                else:
    ...
    def _training_step(
        ...
        acc_list, loss_list = train_probe(
            args,
            train_ds,
            probe,
            dev_dataset=dev_ds,
        ...
        eval_loss, y_true, preds = evaluate_probe(
            args,
            eval_ds,
            probe,
            device=self.device,
        ...
        return {"train": (acc_list, loss_list), "eval": (eval_loss, y_true, preds)}

    def evaluate(
        self,
        train_dataset: Dataset,
        test_dataset: Union[Dataset, Sized],
        ...
        train_datasets_list, eval_datasets_list = self.split_datasets(
            train_dataset, self.fractions, shuffle
        )
        ...
        for index, (train_ds, eval_ds) in tqdm(
            enumerate(zip(train_datasets_list, eval_datasets_list)),
            desc="Online Code training",
            total=len(train_datasets_list),
            disable=not verbose,
        ):
            # print(f"train_ds: {train_ds[0]}")
            # print(f"eval_ds: {eval_ds[0]}")
            ...
            if is_wandb_available():
                wandb.log({"loss": eval_loss, "fraction": fraction})

        res = self._training_step(
            args, train_dataset, dev_dataset, test_dataset, collate_fn
        )
