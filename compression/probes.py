...
def save_checkpoint(
    ...
    path = os.path.join(
        output_dir, "{}_b{}_e{}.pt".format(tag, train_batch_size, num_train_epochs)
    )
    torch.save(
        {
            "epoch": epoch,
...
def load_checkpoint(
    output_dir, train_batch_size, num_train_epochs, model, optimizer, tag="checkpoint"
...
def train_probe(
    ...
    probe_classifier, optimizer, epoch, checkpoint_loss = load_checkpoint(
        checkpoint_path,
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # Load batch
            # ids = batch[0]
            embedding = batch["X"].float().cuda()
            labels = batch["y"].long().cuda()
        ...
        if dev_dataset is not None:
            ...
            dev_loss, _, _ = evaluate_probe(
                args, dev_dataset, probe_classifier, loss_fn=loss_fn, device=device
            )
            train_iterator.set_description(
                "Accuracy: {:.2f}%, Loss: {:.3f}, Validation Loss: {:.3f}".format(
                    acc * 100, mean_loss, dev_loss
                )
            )
            if dev_loss <= min_dev_loss - args.early_stopping_tolerance:
                ...
                if epochs_without_improvement > args.early_stopping:
                    logger.info(
                        f"Early stopping after {epoch} epochs since no validation improvement achieved"
                    )
                    break
        else:
            train_iterator.set_description(
                "Accuracy: {:.2f}%, Loss: {:.4f}".format(acc * 100, mean_loss)
            )
