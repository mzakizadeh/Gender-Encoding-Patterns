...
def create_mappings(ds, mappings_file):
    if Path(mappings_file).exists():
        return

    unique_titles = list(set(r["title"] for r in ds))
    title_mapping = {title: idx for idx, title in enumerate(unique_titles)}
    gender_mapping = {
        gender: idx for idx, gender in enumerate(set(r["gender"] for r in ds))
    }
    ...
    Path(mappings_file).parent.mkdir(parents=True, exist_ok=True)
...
def BiasinBios_split_and_return_tokens_data(
    seed,
    path,
    other=None,
    verbose=False,
    ...
    if verbose:
        print(
            f"X shape: {X.shape}, y shape: {y.shape}, att_masks shape: {att_masks.shape}, z shape: {z.shape}"
        )
    ...
    if other is None:
        (
            ...
            y_train_valid,
            y_test,
            att_masks_train_valid,
            att_masks_test,
            z_train_valid,
            z_test,
            original_y_train_valid,
            original_y_test,
        ) = train_test_split(
...
def BiasinBios_split_and_return_vectors_data(seed, path, verbose=False):
    ...
    return {
        ...
        "test": {"X": X_test, "y": y_test, "z": z_test, "original_y": original_y_test},
        "valid": {
            "X": X_valid,
            "y": y_valid,
            "z": z_valid,
...
def BiasinBios_split_and_save_vectors_data(
    seed, type, verbose=False, folder="../data/biosbias"
):
    ...
    X_train_valid, X_test, y_train_valid, y_test, z_train_valid, z_test = (
        train_test_split(X, y, z, random_state=seed, stratify=y, test_size=0.25)
    )
...
def BiasinBios_balance_tokens_dataset(
    type, seed, oversampling, verbose=False, folder="../data/biosbias"
):
    data_train = torch.load(
        folder + f"/tokens_{type}_original_roberta-base_128_train-seed_{seed}.pt"
    )
    data_valid = torch.load(
        folder + f"/tokens_{type}_original_roberta-base_128_valid-seed_{seed}.pt"
    )
    data_test = torch.load(
        folder + f"/tokens_{type}_original_roberta-base_128_test-seed_{seed}.pt"
    )

    X_train, y_train, z_train, masks_train = (
        data_train["X"],
        data_train["y"],
        data_train["z"],
        data_train["masks"],
    )
    X_valid, y_valid, z_valid, masks_valid = (
        data_valid["X"],
        data_valid["y"],
        data_valid["z"],
        data_valid["masks"],
    )
    X_test, y_test, z_test, masks_test = (
        data_test["X"],
        data_test["y"],
        data_test["z"],
        data_test["masks"],
    ...
    if verbose:
        print(
            ...
            f"y_train shape: {y_train.shape}, "
            f"z_train shape: {z_train.shape}, "
            f"masks_train shape: {masks_train.shape}"
        )
        print(
            f"X_valid shape: {X_valid.shape}, "
            f"y_valid shape: {y_valid.shape}, "
            f"z_valid shape: {z_valid.shape}, "
            f"masks_valid shape: {masks_valid.shape}"
        )
        print(
            f"X_test shape: {X_test.shape}, "
            f"y_test shape: {y_test.shape}, "
            f"z_test shape: {z_test.shape}, "
            f"masks_test shape: {masks_test.shape}"
        )

    X_train_new, y_train_new, z_train_new, masks_train_new = balance_dataset(
        X_train, y_train, z_train, masks=masks_train, oversampling=oversampling
    )
    X_valid_new, y_valid_new, z_valid_new, masks_valid_new = balance_dataset(
        X_valid, y_valid, z_valid, masks=masks_valid, oversampling=oversampling
    )
    X_test_new, y_test_new, z_test_new, masks_test_new = balance_dataset(
        X_test, y_test, z_test, masks=masks_test, oversampling=oversampling
    )
...
def BiasinBios_balance_vectors_dataset(
    type, seed, oversampling, verbose=False, folder="../data/biosbias"
):
    data_train = torch.load(
        folder + f"/vectors_{type}_original_roberta-base_128_train-seed_{seed}.pt"
    ...
    X_train_new, y_train_new, z_train_new = balance_dataset(
        X_train, y_train, z_train, oversampling=oversampling
    )
    X_valid_new, y_valid_new, z_valid_new = balance_dataset(
        X_valid, y_valid, z_valid, oversampling=oversampling
    )
    X_test_new, y_test_new, z_test_new = balance_dataset(
        X_test, y_test, z_test, oversampling=oversampling
    )

    sampling = "oversampled" if oversampling else "subsampled"
    torch.save(
        {"X": X_train_new, "y": y_train_new, "z": z_train_new},
        folder + f"/vectors_{type}_{sampling}_roberta-base_128_train-seed_{seed}.pt",
    )
    torch.save(
        {"X": X_valid_new, "y": y_valid_new, "z": z_valid_new},
        folder + f"/vectors_{type}_{sampling}_roberta-base_128_valid-seed_{seed}.pt",
    )
    torch.save(
        {"X": X_test_new, "y": y_test_new, "z": z_test_new},
...
def BiasinBios_balance_LM_dataset(type, seed, split_type, oversampling, verbose=False):
    data = torch.load(
        f"../data/biosbias/LM_tokens_{type}_original_roberta-base_128_{split_type}-seed_{seed}.pt"
    )
    data2 = torch.load(
        f"../data/biosbias/LM_tokens_{type}_original_roberta-base_128_{split_type}_mask_length_2-seed_{seed}.pt"
    )
    data3 = torch.load(
        f"../data/biosbias/LM_tokens_{type}_original_roberta-base_128_{split_type}_mask_length_3-seed_{seed}.pt"
    )
    data4 = torch.load(
        f"../data/biosbias/LM_tokens_{type}_original_roberta-base_128_{split_type}_mask_length_4-seed_{seed}.pt"
    )
    ...
    X_new, y_new, z_new, gold_sents_new, masks_new, true_length_new = balance_dataset(
        X, y, z, other=(gold_sents, masks, true_length), oversampling=oversampling
    )
    X2_new, y2_new, z2_new, gold_sents2_new, masks2_new, true_length2_new = (
        balance_dataset(
            X2,
            y2,
            z2,
            other=(gold_sents2, masks2, true_length2),
            oversampling=oversampling,
        )
    )
    X3_new, y3_new, z3_new, gold_sents3_new, masks3_new, true_length3_new = (
        balance_dataset(
            X3,
            y3,
            z3,
            other=(gold_sents3, masks3, true_length3),
            oversampling=oversampling,
        )
    )
    X4_new, y4_new, z4_new, gold_sents4_new, masks4_new, true_length4_new = (
        balance_dataset(
            X4,
            y4,
            z4,
            other=(gold_sents4, masks4, true_length4),
            oversampling=oversampling,
        )
    )
...
def balance_dataset(X, y, z, masks=None, other=None, oversampling=False):
    ...
    for label in np.unique(y):
        ...
        if oversampling:
            ...
            if n_female > n_male:
                sampled_female_idx = np.random.choice(
                    female_idx, size=size, replace=False
                )
                sampled_male_idx = np.random.choice(male_idx, size=size, replace=True)
            else:
                sampled_female_idx = np.random.choice(
                    female_idx, size=size, replace=True
                )
...
def BiasinBios_create_percentage_data(split_type, seed):
    data = torch.load(
        f"../data/biosbias/tokens_raw_original_roberta-base_128_{split_type}-seed_{seed}.pt"
    )
    golden_y = data["y"]
    z = data["z"]
    perc = {}
...
def BiasInBios_extract_tokens_data(
    datatype,
    tokenizer,
    model_name,
    data_path,
    max_length,
    experiment_name,
    mappings_file,  # Pass mapping file as an argument
):
    # Load the dataset
    ...
    for i, r in enumerate(tqdm(ds)):
        ...
        inputs[i] = sent
        labels[i] = title_mapping[r["title"]]
        genders[i] = gender_mapping[r["gender"]]
    ...
    attention_masks = np.array(encoded_dict["attention_mask"])

    # Save the processed data
    folder = f"data/Tokenized_Data/{experiment_name}/"
    ...
    torch.save(
        ...
        folder + f"{datatype}_{model_name.replace('/', '-')}_{max_length}.pt",
    )


def BiasInBios_extract_vectors_data(
    type,
    model,
    data_path,
    experiment_name,
    ...
    if Path(
        folder + f"/vectors_{type}_{experiment_name}_{max_length}_layer_1.pt"
    ).exists():
        return
    ...
    # Pre-allocate memory for the vectors, labels, and genders
    vectors = np.zeros((num_layers, num_samples, model.config.hidden_size))
    labels = np.zeros(num_samples, dtype=np.int32)
    genders = np.zeros(num_samples, dtype=np.int32)
    ...
    with torch.no_grad():
        for start_idx in tqdm(range(0, num_samples, batch_size)):
            ...
            batch_input_ids = X[start_idx:end_idx]
            ...
            for j in range(num_layers):
                v = model_output.hidden_states[j + 1][:, 0, :].cpu().detach().numpy()
                vectors[j, start_idx:end_idx, :] = v

            labels[start_idx:end_idx] = y[start_idx:end_idx]
            genders[start_idx:end_idx] = z[start_idx:end_idx]

    # Save all layers at once
    Path(folder).mkdir(parents=True, exist_ok=True)
    for j in range(num_layers):
        torch.save(
            {"X": vectors[j], "y": labels, "z": genders},
            folder + f"/vectors_{type}_{experiment_name}_{max_length}_layer_{j + 1}.pt",
            pickle_protocol=4,
        )


def BiasInBios_extract_last_layer_vectors_data(
    type,
    model,
    data_path,
    experiment_name,
    ...
    # Pre-allocate memory for the vectors, labels, and genders
    vectors = np.zeros((num_samples, model.config.hidden_size))
    labels = np.zeros(num_samples, dtype=np.int32)
    genders = np.zeros(num_samples, dtype=np.int32)
    ...
    with torch.no_grad():
        for start_idx in tqdm(range(0, num_samples, batch_size)):
            ...
            v = model_output.hidden_states[-1][:, 0, :].cpu().detach().numpy()
            vectors[start_idx:end_idx, :] = v
    ...
    Path(folder).mkdir(parents=True, exist_ok=True)
    torch.save(
        {"X": vectors, "y": labels, "z": genders},
        folder + f"/vectors_{type}_{experiment_name}_{max_length}_layer_last.pt",
        pickle_protocol=4,
    )
...
def convert_bias_in_bios_to_LM(biography, tokenizer, mask_len=None, anonymize=False):
    ...
    tokens = tokenizer(f" {article} {title}", add_special_tokens=False)[
        "input_ids"
    ]  # " a dietitian"
    ...
    if anonymize:
        name_replacement = "_"
        return {
            "X": f"{name_replacement} is {masks}."
            + biography["bio"].replace(title, "_"),
            "y": f"{name_replacement} is {article} {title}."
            + biography["bio"].replace(title, "_"),
        ...
        suffix = biography["raw"][biography["start_pos"] :].replace(
            title, "_"
        )  # why replace?
        return {
...
def create_biasinbios_LM_dataset_aux(
    data, datatype, seed, data_type="raw", mask_len=None
):
    ...
    for biography in tqdm(data):
        conversion = convert_bias_in_bios_to_LM(
            biography,
            tokenizer,
            mask_len=mask_len,
...
def create_biasinbios_LM_dataset(path, seed, data_type="raw", mask_len=None):
    ...
    data_train_valid, data_test = train_test_split(
        data, random_state=seed, stratify=y, test_size=0.25
    )
    y_train_valid = [r["title"] for r in data_train_valid]
    data_train, data_valid = train_test_split(
        data_train_valid, random_state=seed, stratify=y_train_valid, test_size=0.133
    )
    print(
        f"train length: {len(data_train)}, valid length: {len(data_valid)}, test length: {len(data_test)}"
    )
